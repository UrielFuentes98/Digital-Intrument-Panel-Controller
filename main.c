...
int main(void)
{
	...
	//FillTest(CanMessages);

while (1) {
	
	readMsgBufID(&id, &len, stmp);
	test = FillDataBase(id,stmp,CanMessages, button);
	//myprintf("\nID:%03X DLC:%d BUFFER:%02X %02X %02X %02X %02X %02X %02X %02X\n", id, len, stmp[0], stmp[1], stmp[2], stmp[3], stmp[4], stmp[5], stmp[6], stmp[7]);
	//printDatapool(CanMessages);
	myprintf("ID = %02x\n",id);
